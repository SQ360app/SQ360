...
export default function RDAPage() {
  ...
  async function createRDA() {
    ...
    const { error } = await supabase.from('rda').insert({
      commessa_id: id, numero, tipo: form.tipo, oggetto: form.oggetto.trim(),
      importo_stimato: parseFloat(form.importo_netto) || 0,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        {[{ l: 'Totale RDA', v: String(totale) }, { l: 'In approvazione', v: String(inApp) }, { l: 'Approvate', v: String(approvate) }, { l: 'Importo stimato', v: 'EUR ' + fmt(importoTot) }].map((k, i) => (
          <div key={i} style={{ background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 600, color: '#111827' }}>{k.v}</div>
          </div>
        ))}
      </div>
      {!showForm && (
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={() => setShowForm(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: '#2563eb', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13 }}>
        ...
        <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 16, marginBottom: 16, background: '#f9fafb' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ gridColumn: '1/-1' }}>
              ...
              <input value={form.oggetto} onChange={e => setForm(p => ({ ...p, oggetto: e.target.value }))} placeholder="Descrizione acquisto..." style={{ width: '100%', padding: '6px 8px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: '#6b7280', display: 'block', marginBottom: 4 }}>Importo stimato EUR</label>
              <input type="number" value={form.importo_netto} onChange={e => setForm(p => ({ ...p, importo_netto: e.target.value }))} style={{ width: '100%', padding: '6px 8px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, textAlign: 'right', boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: '#6b7280', display: 'block', marginBottom: 4 }}>Data necessita</label>
              <input type="date" value={form.data_necessita} onChange={e => setForm(p => ({ ...p, data_necessita: e.target.value }))} style={{ width: '100%', padding: '6px 8px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, boxSizing: 'border-box' }} />
            </div>
            <div style={{ gridColumn: '1/-1' }}>
              <label style={{ fontSize: 12, color: '#6b7280', display: 'block', marginBottom: 4 }}>Note cantiere</label>
              <textarea value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} rows={2} placeholder="Indicazioni tecniche dal cantiere..." style={{ width: '100%', padding: '6px 8px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, resize: 'none', boxSizing: 'border-box' }} />
            </div>
          </div>
          {err && <p style={{ fontSize: 12, color: '#dc2626', margin: '8px 0 0' }}>{err}</p>}
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
            <button onClick={() => { setShowForm(false); setErr('') }} style={{ padding: '6px 14px', fontSize: 13, border: '1px solid #e5e7eb', borderRadius: 6, background: 'white', cursor: 'pointer' }}>Annulla</button>
            <button onClick={createRDA} disabled={saving || !form.oggetto.trim()} style={{ padding: '6px 14px', fontSize: 13, background: '#2563eb', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', opacity: saving || !form.oggetto.trim() ? 0.5 : 1 }}>
              {saving ? '...' : 'Crea RDA'}
            </button>
          </div>
        </div>
      ...
      {loading
        ? <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, gap: 8, color: '#9ca3af', fontSize: 14 }}><Loader2 size={16} className="animate-spin" /> Caricamento...</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {rda.length === 0
                  ...
                  : rda.map(r => {
                      const si = STATI[r.stato]
                      const pr = PRIORITA.find(p => p.v === r.priorita)
                      return (
                        <tr key={r.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                          ...
                          <td style={{ padding: '8px 12px' }}><span style={{ fontSize: 11, fontWeight: 600, color: pr?.c }}>{r.priorita}</span></td>
                          <td style={{ padding: '8px 12px' }}>
                            <span style={{ fontSize: 11, padding: '2px 7px', borderRadius: 10, background: (si?.color || '#9ca3af') + '20', color: si?.color, fontWeight: 600 }}>
                              {si?.label || r.stato}
                            </span>
                          </td>
                          <td style={{ padding: '8px 12px' }}>
