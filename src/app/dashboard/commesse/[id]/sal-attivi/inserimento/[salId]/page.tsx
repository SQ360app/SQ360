...
export default function SalInserimentoPage({ params: paramsPromise }: { params: Promise<{ id: string; salId: string }> }) {
  const { id, salId } = use(paramsPromise)
  ...
  // SAL inserimento
  const [salData, setSalData] = useState<{ id: string; numero: number; stato: string; data_emissione: string }[]>([])
  ...
  const [qtCumulative, setQtCumulative] = useState<Record<string, number>>({})
      ...
      if (sals) {
        ...
        if (otherIds.length > 0) {
          ...
          for (const r of (sv || [])) {
            ...
            perSal[r.sal_id][r.voce_computo_id] = (perSal[r.sal_id][r.voce_computo_id] || 0) + (r.quantita_periodo || 0)
            cumul[r.voce_computo_id] = (cumul[r.voce_computo_id] || 0) + (r.quantita_periodo || 0)
          }
          setQtPerSal(perSal)
          setQtCumulative(cumul)
        }
        // Pre-carica quantità del SAL corrente (riapertura bozza)
        const { data: svCurr } = await supabase
          .from('sal_voci').select('voce_computo_id,quantita_periodo').eq('sal_id', salId)
        if (svCurr && svCurr.length > 0) {
          const init: Record<string, number> = {}
          for (const r of svCurr) init[r.voce_computo_id] = r.quantita_periodo || 0
          setQtInput(init)
        }
      }
  ...
  const salvaVoci = async () => {
    setSaving(true)
    await supabase.from('sal_voci').delete().eq('sal_id', salId)
    const rows = Object.entries(qtInput)
      .filter(([, q]) => q > 0)
      .map(([voceId, q]) => {
        const voce = voci.find(v => v.id === voceId)
        return { sal_id: salId, voce_computo_id: voceId, quantita_periodo: q, wbs_id: voce?.wbs_id ?? null }
      })
    if (rows.length > 0) await supabase.from('sal_voci').insert(rows)
    const importoPeriodo = voci.reduce((s, v) => s + (qtInput[v.id] || 0) * v.prezzo_unitario, 0)
    const cumulPrec = Object.values(qtCumulative).length > 0
      ? voci.reduce((s, v) => s + (qtCumulative[v.id] || 0) * v.prezzo_unitario, 0)
      : 0
    const ritenuta = parseFloat((importoPeriodo * 0.05).toFixed(2))
    await supabase.from('sal').update({
      importo_certificato: parseFloat(importoPeriodo.toFixed(2)),
      importo_cumulativo: parseFloat((cumulPrec + importoPeriodo).toFixed(2)),
      ritenuta_garanzia: ritenuta,
      importo_netto: parseFloat((importoPeriodo - ritenuta).toFixed(2)),
    }).eq('id', salId)
    setSaving(false)
    router.push('../../')
  }
        ...
        <div className="cmp-main">
          ...
          <div className="cmp-tbar">
            ...
            <label style={{ fontSize: 10, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', background: '#4b5563', color: '#fff', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
              📎 PDF DL
              <input type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) console.log('PDF DL:', f.name) }} />
            </label>
            <button className="cmp-tbtn" onClick={() => router.push('../../')}>✕ Annulla</button>
            <button className="cmp-tbtn green" onClick={salvaVoci} disabled={saving}>
              {saving ? '⏳...' : '💾 Salva SAL'}
            </button>
          </div>
              ...
              <table className="cmp-t">
                ...
                <thead>
                  ...
                  <tr>
                    ...
                    {salData.filter(s => s.id !== salId).map(s => (
                    ...
                    <th style={{ background: '#166534', color: '#86efac', fontSize: 9, minWidth: 80 }}>Questo SAL</th>
                    ...
                    if (row.type === 'vo') {
                      ...
                      return (
                        <tr key={v.id} className={`cmp-rvo${isMSel ? ' msel' : isSel ? ' sel' : ''}`}
                          ...
                          <td />
                          {salData.filter(s => s.id !== salId).map(s => (
                            <td key={s.id} style={{ textAlign: 'right', fontFamily: 'monospace', fontSize: 10, color: '#6b7280' }}>
                              {qtPerSal[s.id]?.[v.id] ? qtPerSal[s.id][v.id].toFixed(3) : '—'}
                            </td>
                          ))}
                          <td style={{ textAlign: 'right', fontFamily: 'monospace', fontSize: 10, color: '#374151' }}>
                            {(qtCumulative[v.id] || 0).toFixed(3)}
                          </td>
                          <td style={{ padding: '2px 4px' }} onClick={e => e.stopPropagation()}>
                            <input type="number" step="0.001" min="0"
                              value={qtInput[v.id] || ''}
                              onChange={e => setQtInput(p => ({ ...p, [v.id]: parseFloat(e.target.value) || 0 }))}
                          ...
                          <td style={{ textAlign: 'right', fontSize: 10, fontFamily: 'monospace',
                            color: (v.quantita - (qtCumulative[v.id] || 0) - (qtInput[v.id] || 0)) < 0 ? '#ef4444' : '#16a34a' }}>
                            {(v.quantita - (qtCumulative[v.id] || 0) - (qtInput[v.id] || 0)).toFixed(3)}
                          </td>
                        </tr>
                      )
                    }
                    ...
                    if (row.type === 'som') {
                      ...
                      return (
                        <React.Fragment key={`som_${v.id}`}>
                          ...
                          {isSel && (
                            <tr key={v.id + 'sal-panel'}>
                              <td colSpan={99} style={{ padding: '8px 16px', background: '#eff6ff', borderBottom: '2px solid #bfdbfe' }}>
                                <div style={{ display: 'flex', gap: 24, fontSize: 11 }}>
                                  <span><b>Contratto:</b> {v.quantita.toFixed(3)} {v.um}</span>
                                  <span><b>Cumulativo:</b> {(qtCumulative[v.id] || 0).toFixed(3)} {v.um}</span>
                                  <span><b>Questo SAL:</b> {(qtInput[v.id] || 0).toFixed(3)} {v.um}</span>
                                  <span style={{ color: (v.quantita - (qtCumulative[v.id] || 0) - (qtInput[v.id] || 0)) < 0 ? '#ef4444' : '#16a34a' }}>
                                    <b>Residuo:</b> {(v.quantita - (qtCumulative[v.id] || 0) - (qtInput[v.id] || 0)).toFixed(3)} {v.um}
                                  </span>
                                </div>
                              </td>
                            </tr>
