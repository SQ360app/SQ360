                      ...
                      return (
                        <tr key={v.id} className={`cmp-rvo${isMSel ? ' msel' : isSel ? ' sel' : ''}`}
                          ...
                          <td style={{ textAlign: 'right', fontSize: 10, fontFamily: 'monospace',
                            color: (v.quantita - (props.qtCumulative[v.id] || 0) - (props.qtInput[v.id] || 0)) < 0 ? '#ef4444' : '#16a34a' }}>
                            {(v.quantita - (props.qtCumulative[v.id] || 0) - (props.qtInput[v.id] || 0)).toFixed(3)}
                          </td>
                        </tr>
                      )
                    }
                    ...
                    if (row.type === 'som') {
                      ...
                      return (
                        <React.Fragment key={`som_${v.id}`}>
                          ...
                          {isSel && (() => {
                            const _qtPrec = props.qtCumulative[v.id] || 0
                            const _qtCorr = props.qtInput[v.id] || 0
                            const _residuo = v.quantita - _qtPrec - _qtCorr
                            return (
                              <tr><td colSpan={99} style={{ padding: '8px 16px', background: '#f0f9ff', borderBottom: '2px solid #bfdbfe' }}>
                                <div style={{ display: 'flex', gap: 24, fontSize: 11 }}>
                                  <span><b>Contratto:</b> {v.quantita.toFixed(3)} {v.um}</span>
                                  <span><b>Cumulativo:</b> {_qtPrec.toFixed(3)} {v.um}</span>
                                  <span><b>Questo SAL:</b> {_qtCorr.toFixed(3)} {v.um}</span>
                                  <span style={{ color: _residuo < 0 ? '#ef4444' : '#16a34a' }}>
                                    <b>Residuo:</b> {_residuo.toFixed(3)} {v.um}
                                  </span>
                                </div>
                              </td></tr>
                            )
