...
import { Plus, Trash2, TrendingUp, TrendingDown, Loader2, Save, ChevronDown, ChevronRight } from 'lucide-react'
...
interface Voce { id:string;codice_tariffa?:string;codice?:string;descrizione:string;um:string;quantita:number;prezzo_unitario?:number;importo:number;soggetta_ribasso?:boolean;assegnazioni:Ass[];commessa_id?:string }
...
function VoceRow({voce,fornitori,onRefresh}:{voce:Voce;fornitori:any[];onRefresh:()=>void}) {
  ...
  async function save(){
    ...
    for(const a of ass){
      if(a.isNew){
        await supabase.from('voce_assegnazione').insert({voce_computo_id:voce.id,commessa_id:voce.commessa_id,componente:a.componente,tipo_canale:a.tipo_canale,importo_previsto:a.importo_previsto,fornitore_id:a.fornitore_id||null,stato:a.stato})
      } else {
      ...
      {open&&(
        <div style={{borderTop:'1px solid #f3f4f6',padding:'0 14px 12px'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:6,paddingTop:6,borderTop:'1px solid #f3f4f6'}}>
            <button onClick={()=>setAss(p=>[...p,{id:'new_'+Date.now(),componente:'TOTALE',tipo_canale:'SUBAPPALTO',importo_previsto:Math.max(0,voce.importo-totA),stato:'DA_AVVIARE',isNew:true}])} style={{fontSize:12,color:'#2563eb',background:'none',border:'none',cursor:'pointer',display:'flex',alignItems:'center',gap:3}}><Plus size={12}/>Aggiungi</button>
...
export default function AssegnazioneVociPage() {
  ...
  return (
    <div>
      ...
      {vf.length===0
        ?<div style={{textAlign:'center',padding:'40px',color:'#9ca3af',fontSize:14}}>Nessuna voce trovata</div>
