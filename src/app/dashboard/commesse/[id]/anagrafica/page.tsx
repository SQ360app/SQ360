...
const FIGURE_PROFESSIONALI: FiguraProfessionale[] = [
  // COMMITTENZA / SA
  { ruolo: 'rup', label: 'Responsabile Unico del Procedimento', sigla: 'RUP', gruppo: 'committenza', obbligatorio: true, icona: '⚖️', descrizione: 'D.Lgs. 36/2023 art. 15 — referente unico della SA' },
  { ruolo: 'dl', label: 'Direttore dei Lavori', sigla: 'DL', gruppo: 'committenza', obbligatorio: true, icona: '📐', descrizione: 'Sorveglianza tecnica, accettazione materiali, SAL' },
  { ruolo: 'direttore_operativo', label: 'Direttore Operativo', sigla: 'DO', gruppo: 'committenza', obbligatorio: false, icona: '🔭', descrizione: 'Assistente DL per specifici settori lavorativi' },
  { ruolo: 'ispettore_cantiere', label: 'Ispettore di Cantiere', sigla: 'IC', gruppo: 'committenza', obbligatorio: false, icona: '🔍', descrizione: 'Controllo quotidiano in cantiere per conto del DL' },
  { ruolo: 'csp', label: 'Coordinatore Sicurezza Progettazione', sigla: 'CSP', gruppo: 'committenza', obbligatorio: true, icona: '🦺', descrizione: "D.Lgs. 81/2008 — redige PSC, fascicolo dell'opera" },
  { ruolo: 'cse', label: 'Coordinatore Sicurezza Esecuzione', sigla: 'CSE', gruppo: 'committenza', obbligatorio: true, icona: '🚧', descrizione: 'Verifica applicazione PSC, sospende lavori se necessario' },
  { ruolo: 'collaudatore_co', label: "Collaudatore in Corso d'Opera", sigla: 'COLL-CO', gruppo: 'committenza', obbligatorio: false, icona: '📋', descrizione: 'Per lavori con SAL intermedi di importo rilevante' },
  { ruolo: 'collaudatore_finale', label: 'Collaudatore Finale / Statico', sigla: 'COLL-F', gruppo: 'committenza', obbligatorio: true, icona: '✅', descrizione: 'Verifica conformità opera ultimata al progetto' },
  { ruolo: 'progettista', label: 'Progettista', sigla: 'PROG', gruppo: 'committenza', obbligatorio: false, icona: '✏️', descrizione: 'Progettazione architettonica generale' },
  { ruolo: 'progettista_strutturale', label: 'Progettista Strutturale', sigla: 'PROG-S', gruppo: 'committenza', obbligatorio: false, icona: '🏗️', descrizione: 'Calcolo strutturale, certificazioni NTC 2018' },
  { ruolo: 'progettista_impiantistico', label: 'Progettista Impiantistico', sigla: 'PROG-I', gruppo: 'committenza', obbligatorio: false, icona: '⚡', descrizione: 'Impianti elettrici, termici, idraulici' },
  { ruolo: 'geologo', label: 'Geologo', sigla: 'GEO', gruppo: 'committenza', obbligatorio: false, icona: '🪨', descrizione: 'Relazione geologica e geotecnica' },
  // IMPRESA
  { ruolo: 'responsabile_commessa', label: 'Responsabile di Commessa', sigla: 'RC', gruppo: 'impresa', obbligatorio: true, icona: '👷', descrizione: 'Referente lato impresa, approva SAL passivi' },
  { ruolo: 'capocantiere', label: 'Capocantiere', sigla: 'CAP', gruppo: 'impresa', obbligatorio: true, icona: '🦺', descrizione: 'Gestione operativa quotidiana, rapportini' },
  { ruolo: 'rspp', label: 'RSPP', sigla: 'RSPP', gruppo: 'impresa', obbligatorio: true, icona: '🛡️', descrizione: 'Responsabile Servizio Prevenzione e Protezione' },
  { ruolo: 'assistente_cantiere', label: 'Assistente di Cantiere', sigla: 'ASS', gruppo: 'impresa', obbligatorio: false, icona: '📝', descrizione: 'Supporto tecnico-amministrativo in cantiere' },
  { ruolo: 'preposto', label: 'Preposto', sigla: 'PREP', gruppo: 'impresa', obbligatorio: false, icona: '👁️', descrizione: "D.Lgs. 81/2008 — sorveglianza sicurezza operai" },
]
    ...
    return (
      <div className="flex items-center gap-3 bg-slate-800 border border-slate-600 rounded-lg p-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-white">{valore.nome} {valore.cognome}</p>
        ...
        <button onClick={() => onSeleziona({ id: '', nome: '', cognome: '', tipo: '' })} className="text-slate-500 hover:text-red-400 transition-colors">
          <X size={14} />
        </button>
      </div>
    )
  ...
  return (
    <div ref={dropRef} className="relative">
      <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
        {caricamento ? <Loader2 size={14} className="text-blue-400 animate-spin" /> : <Search size={14} className="text-slate-500" />}
      </div>
      <input
        type="text"
        value={query}
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 border border-slate-600 rounded-lg shadow-2xl shadow-black/50 z-50 overflow-hidden">
          {risultati.length > 0 ? (
            <>
              {risultati.map(p => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white font-medium">{p.nome} {p.cognome}</p>
                    <p className="text-xs text-slate-400 truncate">
                      {p.ordine_professionale ? `${p.ordine_professionale} · ${p.numero_iscrizione || '—'}` : p.tipo}
                    </p>
                  </div>
                </button>
              ))}
              ...
              >
                <Plus size={12} /> Aggiungi nuovo professionista al database
              </button>
            </>
          ) : query.length >= 2 ? (
            <div className="px-3 py-3">
              ...
              >
                <Plus size={12} /> Aggiungi &quot;{query}&quot; come nuovo professionista
              </button>
            </div>
          ) : (
            <p className="px-3 py-3 text-xs text-slate-500">Digita almeno 2 caratteri per cercare...</p>
          )}
        </div>
      )}
    </div>
...
function ModalNuovoProfessionista({
  ...
  const f = (k: keyof Professionista, v: string) => setForm(prev => ({ ...prev, [k]: v }))

  useEffect(() => {
    if (aperto) {
      const parti = queryIniziale.trim().split(' ')
      setForm({ nome: parti[0] || '', cognome: parti.slice(1).join(' ') || '', tipo: 'professionista' })
    }
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-slate-700">
          <h3 className="text-lg font-semibold text-white flex items-center gap-2">
            <Plus size={18} className="text-blue-400" /> Nuovo professionista / collaboratore
          </h3>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.cognome || ''} onChange={e => f('cognome', e.target.value)}
                className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
                placeholder="Rossi" />
            </div>
          </div>
          {form.tipo === 'professionista' && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.ordine_professionale || ''} onChange={e => f('ordine_professionale', e.target.value)}
                  className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
                  placeholder="Ingegnere Civile - OAI NA" />
              </div>
              <div>
                <label className="text-xs text-slate-400 mb-1 block">N. iscrizione</label>
                <input value={form.numero_iscrizione || ''} onChange={e => f('numero_iscrizione', e.target.value)}
                  className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.pec || ''} onChange={e => f('pec', e.target.value)} type="email"
                className="w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500"
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-slate-700">
          <button onClick={onChiudi} className="px-4 py-2 text-sm text-slate-400 hover:text-white transition-colors">
            Annulla
          </button>
...
export default function AnagraficaCommessa({
  ...
  const [commessa, setCommessa] = useState<{
    ...
    nome?: string
    figure: Figure
  }>({ sicurezza_interna_esclusa_ribasso: true, figure: {} })
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        for (const f of (data.commessa_figure || [])) {
          figure[f.ruolo] = { professionista_id: f.professionista_id, professionista: f.professionista }
        }
  ...
  const salvaFigura = (ruolo: string, p: Professionista) => {
    if (!p.id) {
      setCommessa(prev => { const f = { ...prev.figure }; delete f[ruolo]; return { ...prev, figure: f } })
      return
    }
    setCommessa(prev => ({
      ...prev,
      figure: { ...prev.figure, [ruolo]: { professionista_id: p.id, professionista: p } }
    }))
  }
  ...
  const salvaCommessa = async () => {
    ...
    try {
      const { error } = await supabase
        .from('commesse')
        .update({ sicurezza_interna_esclusa_ribasso: commessa.sicurezza_interna_esclusa_ribasso })
        .eq('id', id)
      if (error) throw error

      await supabase.from('commessa_figure').delete().eq('commessa_id', id)
      const ins = Object.entries(commessa.figure)
        .filter(([, v]) => v.professionista_id)
        .map(([ruolo, v]) => ({ commessa_id: id, ruolo, professionista_id: v.professionista_id }))
      if (ins.length) await supabase.from('commessa_figure').insert(ins)
      ...
      <div className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur border-b border-slate-800 px-6 py-4">
        <div className="flex items-center justify-between max-w-5xl mx-auto">
          <div>
            ...
            <p className="text-sm text-slate-400">{commessa.codice} — {commessa.nome || '—'}</p>
          </div>
          <button
            onClick={salvaCommessa}
            disabled={salvando}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${
              salvato ? 'bg-green-600 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'
            } disabled:opacity-60`}
          >
            {salvato
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-slate-900 border border-amber-700/30 rounded-2xl p-5">
          <h2 className="text-sm font-semibold text-amber-400 mb-1 flex items-center gap-2">
            <Shield size={15} /> Configurazione ribasso sicurezza
          </h2>
          <p className="text-xs text-slate-500 mb-4">
            Trattamento quota sicurezza interna alle voci (Strato 3) per questa commessa
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
              ...
              <p className="text-xs text-green-300 mt-2">✓ Ribasso: sempre</p>
            </div>
            {/* Strato 2 */}
            <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
              <p className="text-xs font-semibold text-red-400 mb-1">Strato 2 — Sicurezza speciale</p>
              <p className="text-xs text-slate-400">Voci sicurezza dedicate</p>
              <p className="text-xs text-red-300 mt-2">✗ Ribasso: mai (fisso)</p>
            </div>
            ...
            <div className={`rounded-xl p-4 border transition-colors ${
              ...
              <p className="text-xs font-semibold text-amber-400 mb-1">Strato 3 — Sicurezza interna</p>
              <p className="text-xs text-slate-400 mb-3">Quota IncSIC in ogni EPItem (XPWE)</p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setCommessa(p => ({ ...p, sicurezza_interna_esclusa_ribasso: !p.sicurezza_interna_esclusa_ribasso }))}
                ...
                <span className="text-xs text-slate-300">
                  {commessa.sicurezza_interna_esclusa_ribasso ? 'Esclusa dal ribasso' : 'Soggetta a ribasso'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 bg-slate-950 rounded-xl p-4 border border-slate-800 font-mono text-xs">
            <p className="text-slate-500 mb-1">Formula voce applicata:</p>
            <p className="text-slate-300">
              <span className="text-green-400">IMPORTO</span> = Q×PU×(1−IncSIC)×(1−R)
              <span className="text-slate-500"> ← lavori ribassati</span>
            </p>
            <p className="text-slate-300">
              {commessa.sicurezza_interna_esclusa_ribasso
                ? <><span className="text-amber-400">+ Q×PU×IncSIC</span><span className="text-slate-500"> ← sic. interna NON ribassata</span></>
                : <><span className="text-slate-400">+ Q×PU×IncSIC×(1−R)</span><span className="text-slate-500"> ← sic. interna ribassata</span></>
              }
            </p>
            <p className="text-slate-300">
        ...
        {/* ── GRUPPI FIGURE ── */}
        {([
          { key: 'committenza' as const, label: 'Stazione Appaltante / Committente', icona: <Building2 size={15} className="text-blue-400" />, bg: 'bg-blue-600/20', border: 'border-blue-500/30', figure: figureCommittenza },
          { key: 'impresa' as const, label: 'Impresa Esecutrice', icona: <HardHat size={15} className="text-orange-400" />, bg: 'bg-orange-600/20', border: 'border-orange-500/30', figure: figureImpresa },
        ]).map(gruppo => (
          <div key={gruppo.key} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
            <button
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg ${gruppo.bg} border ${gruppo.border} flex items-center justify-center`}>
                  {gruppo.icona}
                </div>
                <div className="text-left">
                  <h2 className="text-sm font-semibold text-white">{gruppo.label}</h2>
                  <p className="text-xs text-slate-500">
                    {Object.keys(commessa.figure).filter(r => gruppo.figure.find(f => f.ruolo === r)).length} / {gruppo.figure.length} figure assegnate
                  </p>
                </div>
              </div>
                ...
                {gruppo.figure.map(figura => (
                  <div key={figura.ruolo} className="space-y-1.5">
                    ...
                    <RicercaProfessionista
                      figura={figura}
                      valore={commessa.figure[figura.ruolo]?.professionista}
                      onSeleziona={p => salvaFigura(figura.ruolo, p)}
                      onAggiungiNuovo={r => setModal({ aperto: true, ruolo: r, query: '' })}
                    />
                  </div>
                ))}
              </div>
