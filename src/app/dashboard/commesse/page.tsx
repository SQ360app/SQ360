...
interface FormCommessa {
  ...
  dl_email: string
  dl_telefono: string
  direttore_operativo_nome: string
  direttore_operativo_email: string
  ispettore_cantiere_nome: string
  ispettore_cantiere_email: string
  csp_nome: string
  csp_email: string
  cse_nome: string
  cse_email: string
  collaudatore_nome: string
  collaudatore_email: string
  collaudatore_statico_nome: string
  collaudatore_statico_email: string
  rc_nome: string
  rc_email: string
  rc_telefono: string
  direttore_tecnico_nome: string
  direttore_tecnico_email: string
  capocantiere_nome: string
  capocantiere_telefono: string
  rspp_nome: string
  rspp_email: string
  preposto_nome: string
  preposto_telefono: string
  responsabile_qualita_nome: string
  responsabile_qualita_email: string
}
...
export default function CommessePage() {
  ...
  async function handleFileImport(file: File) {
    if (file.name.toLowerCase().endsWith('.pdf') || file.type === 'application/pdf') {
      setStep('FORM')
      setAiOk(false)
      setAiMsg('⚠️ PDF non supportati — usa TXT o DOCX, oppure compila manualmente')
      return
    }
    setStep('AI_LOADING')
    ...
    try {
      ...
      if (testo.trim().length < 30) {
        setStep('FORM')
        setAiOk(false)
        setAiMsg('⚠️ Documento vuoto o non leggibile — compila manualmente')
        return
      }
      ...
      if (json.ok && json.dati) {
        ...
        const prov = str(d.provincia) || 'NA'
        const cat = (str(d.categoria_prevalente) || 'GE').toUpperCase().slice(0, 2)
        setForm(p => ({
          ...p,
          codice: generaCodice(prov, cat, commesse.length + 1),
          nome: str(d.nome) || p.nome,
          committente: str(d.committente) || p.committente,
          cig: str(d.cig) || p.cig,
          cup: str(d.cup) || p.cup,
          importo_base: num(d.importo_base) || p.importo_base,
          importo_aggiudicato: num(d.importo_aggiudicato) || p.importo_aggiudicato,
          ribasso_pct: num(d.ribasso_pct) || p.ribasso_pct,
          oneri_sicurezza: num(d.oneri_sicurezza) || p.oneri_sicurezza,
          provincia: prov, categoria: cat,
          tipo_committente: str(d.tipo_committente) || p.tipo_committente,
          data_aggiudicazione: str(d.data_aggiudicazione) || p.data_aggiudicazione,
          durata_giorni: parseInt(String(d.durata_giorni || '365')) || p.durata_giorni,
          indirizzo_cantiere: str(d.indirizzo_cantiere) || p.indirizzo_cantiere,
          citta_cantiere: str(d.citta_cantiere) || p.citta_cantiere,
          rup_nome: str(d.rup_nome) || p.rup_nome,
          rup_email: str(d.rup_email) || p.rup_email,
          dl_nome: str(d.dl_nome) || p.dl_nome,
          dl_email: str(d.dl_email) || p.dl_email,
          note: str(d.note) || p.note,
        }))
        setAiOk(true)
        setAiMsg('✅ Dati estratti — verifica e integra i campi mancanti')
      } else {
        setAiOk(false)
        setAiMsg(`⚠️ ${str(d?.errore) || json.errore || 'Estrazione parziale'} — compila i campi mancanti`)
      }
    } catch (e) {
      setAiOk(false)
  ...
  async function creaCommessa() {
    ...
    try {
      ...
      if (ut.user?.id) {
        const { data: utData } = await supabase
          .from('utenti').select('azienda_id').eq('id', ut.user.id).single()
    ...
    const { data, error } = await supabase
      .from('commesse').insert([payload]).select('id,codice').single()

    setSaving(false)
    if (error) {
      setErroreInsert(`Errore DB: ${error.message} (${error.code})`)
      return
    }
    if (data) {
      setShowNuova(false)
  ...
  const filtrate = commesse.filter(c => {
    ...
    const mq = !search || [c.nome, c.codice, c.committente]
      .some(x => x?.toLowerCase().includes(search.toLowerCase()))
    return ms && mq
  })
  const portafoglio = commesse
    .filter(c => c.stato !== 'CHIUSA')
    .reduce((s, c) => s + (c.importo_aggiudicato || 0), 0)
  ...
  const figSA: Array<{ k: keyof FormCommessa; l: string }> = [
    ...
    { k: 'collaudatore_nome', l: 'Collaudatore T.A.' }, { k: 'collaudatore_email', l: 'Coll. T.A. Email' },
    { k: 'collaudatore_statico_nome', l: 'Coll. Statico' }, { k: 'collaudatore_statico_email', l: 'Coll. Stat. Email' },
  ]
  const figImpresa: Array<{ k: keyof FormCommessa; l: string }> = [
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          {['TUTTI', ...STATI].map(s => (
            <button key={s} onClick={() => setFiltroStato(s)}
              style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 11, fontWeight: 600, cursor: 'pointer', background: filtroStato === s ? (STATO_COLOR[s] || 'var(--accent)') : 'var(--panel)', color: filtroStato === s ? 'white' : 'var(--t3)', whiteSpace: 'nowrap' }}>
            ...
            {step === 'UPLOAD' && (
              <>
                ...
                <div onClick={() => fileRef.current?.click()}
                  style={{ border: '2px dashed #e2e8f0', borderRadius: 14, padding: '40px 24px', textAlign: 'center', cursor: 'pointer', background: '#f8fafc', marginBottom: 16 }}>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div>
                    ...
                    {aiMsg && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5, fontSize: 11, color: aiOk === true ? '#10b981' : '#f59e0b' }}>
                        {aiOk === true ? <CheckCircle size={12} /> : <AlertCircle size={12} />}
                        {aiMsg}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        <button onClick={aggiornaCodice} title="Rigenera"
                          style={{ padding: '8px 10px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg)', cursor: 'pointer', color: 'var(--accent)' }}>↺</button>
                      </div>
                    ...
                    <div>
                      ...
                      <select value={form.categoria} onChange={e => setStr('categoria', e.target.value)} style={{ ...inp, width: '100%' }}>
                        {[['GE','Generale'],['RS','Ristrutturazione'],['NR','Nuova Realizzazione'],['ML','Manutenzione'],['SI','Impianti'],['ST','Strade'],['ID','Idraulica'],['EL','Elettrico']].map(([v, l]) => <option key={v} value={v}>{v} — {l}</option>)}
                      </select>
                    </div>
                    ...
                    <div>
                      <label style={{ ...lbl, display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <button onClick={geocodifica} style={{ fontSize: 9, color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700 }}>📍 Calcola coordinate</button>
                      </label>
                      <div style={{ display: 'flex', gap: 6 }}>
