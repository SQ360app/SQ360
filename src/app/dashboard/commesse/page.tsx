...
export default function CommessePage() {
  ...
  async function carica() {
    setLoading(true)
    const { data } = await supabase.from('v_commesse_kpi').select('id,codice,nome,committente,stato,importo_aggiudicato,avanzamento_pct').order('stato')
  ...
  async function handleFileImport(file: File) {
    ...
    try {
      ...
      if (json.ok && json.dati) {
        ...
        const prov = s(d.provincia)||'NA'
        const cat = (s(d.categoria_prevalente)||'GE').toUpperCase().slice(0,2)
        setForm(p => ({
          ...p,
          codice: generaCodice(prov, cat, commesse.length+1),
          nome: s(d.nome)||p.nome, committente: s(d.committente)||p.committente,
          cig: s(d.cig)||p.cig, cup: s(d.cup)||p.cup,
          importo_base: n(d.importo_base)||p.importo_base,
          importo_aggiudicato: n(d.importo_aggiudicato)||p.importo_aggiudicato,
          ...
          rup_nome: s(d.rup_nome)||p.rup_nome, rup_email: s(d.rup_email)||p.rup_email,
          dl_nome: s(d.dl_nome)||p.dl_nome, dl_email: s(d.dl_email)||p.dl_email,
        }))
        setAiOk(true); setAiMsg('✅ Dati estratti — verifica e integra i campi mancanti')
      } else {
        setAiOk(false); setAiMsg(`⚠️ ${json.errore||'Estrazione parziale'} — compila manualmente`)
      }
  ...
  return (
    <div style={{padding:'22px 28px',background:'var(--bg)',minHeight:'100%'}}>
      ...
      <div className="card" style={{overflow:'hidden'}}>
        ...
        : filtrate.length===0 ? <div style={{padding:'60px 32px',textAlign:'center',color:'var(--t3)',fontSize:13}}>{commesse.length===0?'Nessuna commessa — clicca "+ Nuova commessa"':'Nessun risultato'}</div>
        : filtrate.map((c,i)=>{
        ...
        <div className="modal-overlay">
          <div className="modal-box" style={{maxWidth:step==='FORM'?780:480}}>
            ...
            {step==='FORM' && <>
              ...
              {erroreInsert && <div style={{background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.25)',borderRadius:8,padding:'10px 14px',marginBottom:14,fontSize:12,color:'#ef4444',display:'flex',gap:8}}><AlertTriangle size={14} style={{flexShrink:0,marginTop:1}} />{erroreInsert}</div>}
              ...
              <div style={{maxHeight:'62vh',overflowY:'auto',paddingRight:4}}>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                  ...
                  <div style={{gridColumn:'span 2',marginTop:8}}>
                    ...
                    {showFigure && <div style={{border:'1px solid #e2e8f0',borderRadius:10,marginTop:4,overflow:'hidden'}}>
                      <button onClick={()=>setSezFigure('sa')} style={{width:'100%',display:'flex',justifyContent:'space-between',padding:'9px 14px',background:sezFigure==='sa'?'rgba(59,130,246,0.06)':'#f8fafc',border:'none',borderBottom:'1px solid #e2e8f0',cursor:'pointer',fontSize:11,fontWeight:700,color:'#334155'}}>
                        🏛 Stazione Appaltante (RUP, DL, CSP, CSE...) <ChevronDown size={12}/>
                      </button>
                      {sezFigure==='sa' && <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,padding:'12px 14px',background:'#fafafa'}}>
                      ...
                      <button onClick={()=>setSezFigure('impresa')} style={{width:'100%',display:'flex',justifyContent:'space-between',padding:'9px 14px',background:sezFigure==='impresa'?'rgba(59,130,246,0.06)':'#f8fafc',border:'none',borderTop:'1px solid #e2e8f0',cursor:'pointer',fontSize:11,fontWeight:700,color:'#334155'}}>
                        🏗 Impresa Esecutrice (RC, Capocantiere, RSPP...) <ChevronDown size={12}/>
                      </button>
                      {sezFigure==='impresa' && <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,padding:'12px 14px',background:'#fafafa'}}>
              ...
              {(!form.nome.trim()||!form.committente.trim()) && <div style={{textAlign:'center',marginTop:8,fontSize:11,color:'#94a3b8'}}>Compila <strong>Nome lavori</strong> e <strong>Committente</strong></div>}
            </>}
          </div>
        </div>
      )}
